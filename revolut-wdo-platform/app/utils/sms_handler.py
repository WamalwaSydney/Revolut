...
class SMSHandler:
    ...
    def process_ussd_request(self, session_id, service_code, phone_number, text):
        ...
        if session.get('step') == 'main_menu':
            if current_step == '1':
                ...
                # This would come from DB in real implementation
            ...
            # Save feedback to database (simplified)
            feedback_text = current_step
            # In real implementation, create Feedback object and save
            response = "END Thank you! Your feedback has been submitted. Reference: #12345"
            # Clear session
            if session_id in self.ussd_session:
                del self.ussd_session[session_id]
            return response
    ...
    def send_sms(self, phone_number, message):
        ...
        try:
            ...
            sender = current_app.config.get('SMS_SENDER_ID', 'REVOLUT')
            response = self.africas_talking.send(message, recipients, sender)
            return response
