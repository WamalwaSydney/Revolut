...
from app.models import User, Feedback, Poll, FeedbackResponse,PollOption
from app import db
...
@admin.route('/polls/create', methods=['GET', 'POST'])
def create_poll():
    """Create a new poll"""
    if request.method == 'POST':
        title = request.form.get('title')
        description = request.form.get('description')
        options = request.form.getlist('options')  # receives multiple inputs with same name

        if not title or not options or len(options) < 2:
            flash('Poll must have a title and at least 2 options.', 'error')
            return redirect(url_for('admin.create_poll'))

        poll = Poll(title=title, description=description, is_active=True)
        db.session.add(poll)
        db.session.flush()  # to get poll.id before commit
